...
class typedechetviewmodel : ViewModel() {
    // Private MutableLiveData
    private val _listTypeLiveData = MutableLiveData<List<Type>>()

    // Public LiveData
    val listTypeLiveData: LiveData<List<Type>> get() = _listTypeLiveData
            ...
            override fun onResponse(call: Call<TypeList>, response: Response<TypeList>) {
                if (response.isSuccessful) {
                    response.body()?.let { typeList ->
                        _listTypeLiveData.postValue(typeList)
